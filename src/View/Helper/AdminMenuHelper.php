 ...
 */
    ...
    public function render($data, $level = 0, $options = []){
        ...
        $html = '';
        $data = Hash::sort(Hash::filter($data), '{*}.weight', 'ASC');
        foreach($data as $class=>$menu){
            ...
            $isDropdown = false;
            $liOptions = ['class' => ''];
            if(($level == 0)&&Hash::get($menu, 'options.dropdown', false)){
                $isDropdown = true;
    ...
    protected function link($link, $level = 0){
        $isDropdown = false;
        if(($level == 0)&&Hash::get($link, 'options.dropdown', false)){
            $isDropdown = true;
        }
        ...
        if(!is_string($link['url'])){
            $link = Hash::insert($link, 'url.plugin', Hash::get($link, 'url.plugin', false));
        }
